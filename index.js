...
if (fs.existsSync(DB_FILE)) {
  try {
    ...
    // إذا كانت قاعدة البيانات القديمة لا تحتوي chatHealth
    if (!db.chatHealth) {
      db.chatHealth = {
        violations: 0,
        lastViolation: null,
    ...
    if (typeof db.chatHealth.violations !== "number") {
      db.chatHealth.violations = 0;
    }

    if (!("lastViolation" in db.chatHealth)) {
      db.chatHealth.lastViolation = null;
    }

    if (!("lastUser" in db.chatHealth)) {
      db.chatHealth.lastUser = null;
    }

    if (!("lastChannel" in db.chatHealth)) {
      db.chatHealth.lastChannel = null;
    }

    if (!("lastTime" in db.chatHealth)) {
      db.chatHealth.lastTime = null;
    }
...
function getChatHealth() {
  ...
  return {
    ...
    description:
      `⚠️ تم رصد **${health.violations}** مخالفة.\n\n` +
      `👤 آخر مخالف: <@${health.lastUser}>\n` +
      `📍 القناة: <#${health.lastChannel}>\n` +
      `🕐 الوقت: <t:${Math.floor(health.lastTime / 1000)}:R>\n\n` +
      `🛡️ تم التعامل مع المخالفة تلقائيًا.`
  ...
  async () => {
    ...
    if (guild) {
      ...
      await botLog(
        ...
        `📊 مخالفات الشات المحفوظة: **${db.chatHealth.violations}**`
      );
...
client.on(
  ...
  async message => {
    ...
    db.chatHealth.lastViolation =
      message.content;

    db.chatHealth.lastUser =
      message.author.id;

    db.chatHealth.lastChannel =
      message.channel.id;

    db.chatHealth.lastTime =
      Date.now();

    // حفظ فوري في database.json
    saveDB();
    ...
    const warningEmbed =
      new EmbedBuilder()
        ...
        .setDescription(
          ...
          `📊 إجمالي المخالفات المرصودة: **${db.chatHealth.violations}**`
        )
        .setTimestamp();
    ...
    await modLog(
      ...
      `📊 إجمالي المخالفات: **${db.chatHealth.violations}**`
